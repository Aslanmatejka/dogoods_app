    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          full_address: { type: 'string', description: 'Address for pickup' },
          pickup_by: { type: 'string', description: 'Pickup deadline (ISO 8601 datetime)' },
  ...
  switch (toolName) {
    case 'search_food_nearby': {
      ...
      let query = supabaseClient
        .from('food_listings')
        .select('id, title, description, category, quantity, full_address, pickup_by, expiry_date, dietary_tags, latitude, longitude, created_at, status')
    ...
    case 'get_user_profile': {
      ...
      const { data, error } = await supabaseClient
        .from('users')
        .select('id, name, email, dietary_restrictions, location, phone, avatar_url, is_admin, created_at')
        .eq('id', userId)
        .single()
    ...
    case 'get_pickup_status': {
      ...
      const { data, error } = await supabaseClient
        .from('food_claims')
        .select('id, status, created_at, notes, food_listings(id, title, description, category, full_address, pickup_by, expiry_date)')
        .eq('user_id', userId)
    ...
    case 'get_my_listings': {
      ...
      const { data, error } = await supabaseClient
        .from('food_listings')
        .select('id, title, description, category, quantity, status, full_address, pickup_by, expiry_date, created_at')
        .eq('user_id', userId)
        .order('created_at', { ascending: false })
        .limit(10)
    ...
    case 'create_food_listing': {
      if (!userId) return { error: 'User not authenticated' }
      const { title, description, category, quantity, full_address, pickup_by, expiry_date, dietary_tags } = args
    ...
    if (authHeader) {
      ...
      if (!authError && user) {
        ...
        const { data: profile } = await supabaseAdmin
          .from('users')
          .select('name, email, dietary_restrictions, location, phone, is_admin')
          .eq('id', userId)
          .single()
